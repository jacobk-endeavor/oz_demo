 ...
 * re-running TTS. Run from repo root: node lumberyard-calls/tools/refresh-durations.mjs
 */
...
const MANIFEST = join(REPO_ROOT, 'lumberyard-calls', 'call-library.json')

const raw = JSON.parse(readFileSync(MANIFEST, 'utf8'))
raw.calls = raw.calls.map((c) => {
  if (!c.audio) return c
  const rel = c.audio.replace(/^lumberyard-calls\//, 'lumberyard-calls/')
