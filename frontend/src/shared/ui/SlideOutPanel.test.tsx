...
describe('SlideOutPanel', () => {
  ...
  it('opens with registry body for table kind and closes via button', async () => {
    ...
    expect(screen.getByRole('dialog', { name: /oz panel: table/i })).toBeInTheDocument()
  ...
  it('uses unsupported fallback for unknown kinds', () => {
    render(
      <SlideOutPanel open onClose={() => {}} panelKind="unknown_xyz" payload={{ hint: 1 }} />,
    )
  ...
  it('keeps Tab focus inside the dialog', async () => {
    ...
    closeBtn.focus()
    await user.tab()
    const dialog = screen.getByRole('dialog', { name: /oz panel: table/i })
  ...
  it('uses bottom-sheet layout attributes when viewport matches mobile breakpoint', () => {
    mockMatchMedia(true)
    const { container } = render(
      <SlideOutPanel open onClose={() => {}} panelKind="chart" payload={{}} />,
    )
