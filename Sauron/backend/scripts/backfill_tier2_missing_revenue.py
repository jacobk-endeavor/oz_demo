"""Backfill company profiles for Tier 2 leads with missing revenue.

This reruns the company profile enrichment only for Tier 2 leads whose
``revenue_m`` is null or blank.

Usage:
    uv run python scripts/backfill_tier2_missing_revenue.py
    uv run python scripts/backfill_tier2_missing_revenue.py --max-concurrency 5
    uv run python scripts/backfill_tier2_missing_revenue.py --dry-run
"""
...
_BOOTSTRAP_ENV_VAR = "SAURON_TIER2_REVENUE_BACKFILL_BOOTSTRAPPED"
...
def _missing_revenue(profile: LeadCompanyProfile | None) -> bool:
    if profile is None:
        return True
...
async def _count_missing_revenue() -> int:
    async with async_session() as db:
        return (
            await db.execute(
                select(func.count())
                ...
                .where(
                    ...
                    or_(
                        LeadCompanyProfile.revenue_m.is_(None),
...
async def _main(max_concurrency: int, dry_run: bool) -> None:
    try:
        ...
        targets = [(lead.id, lead.company) for lead in leads if _missing_revenue(lead.profile)]
        print(f"Found {len(targets)} Tier 2 leads with missing revenue.")
        ...
        progress = tqdm(total=len(tasks), desc="Backfilling Tier 2 revenue", unit="lead")
        try:
        ...
        remaining_missing = await _count_missing_revenue()
        print(
            f"\nDone: total={len(tasks)} updated={succeeded} "
            f"skipped={skipped} failed={failed} remaining_missing_revenue={remaining_missing}"
        )
    finally:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Backfill company profiles for Tier 2 leads with missing revenue."
    )
    parser.add_argument(
        "--max-concurrency",
        type=int,
        default=5,
        help="Maximum number of leads to enrich concurrently (default: 5).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="List Tier 2 leads with missing revenue without running enrichment.",
    )
