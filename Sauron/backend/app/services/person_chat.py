...
The user is viewing a person's profile page that aggregates all meeting transcripts \
this person has participated in.
...
async def stream_person_chat(
    *,
    ...
    scope_filters = {"person_ids": [person_id]} if person_id is not None else None

    async for chunk in stream_chat_sse(
        system_prompt=_SYSTEM_PROMPT,
        context_block=f"Person: {person_name}\n\n{transcripts_block}",
        messages=messages,
        current_scope_label="this person's transcripts",
        current_scope_filters=scope_filters,
        db=db,
        user_role=user_role,
        user_email=user_email,
        error_label=f"Person chat for {person_name}",
    ):
        yield chunk
